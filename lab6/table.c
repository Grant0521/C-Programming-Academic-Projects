 ...
 * Complexity: O(n)
 *
 * Description:	Finds and uses a pivot to sort subsections of elements
 */


int partition(void **array, int low, int high, SET *sp) {
    assert(array != NULL);
    int pivIndex, j, i;
    void *temp;
    ...
    j = low;
    for (i = low; i < high; i++) {
      if ((*sp->compare)(array[i], array[pivIndex]) < 0) {
	    temp = array[i];
	    array[i] = array[j];
	    array[j] = temp;
	    j++;
 ...
 * Complexity: O(n^2)
 *
 * Description: Calls the partition function to sort subsections
 *              that are recursively called
 */

void quickSort(void **array, int low, int high, SET *sp) {
    assert(array != NULL);
    int i;

    if (high > low) {
      i = partition(array, low, high, sp);
      quickSort(array, low, i - 1, sp);
      quickSort(array, i + 1, high, sp);
    }
}
 ...
 *
 * Complexity:	O(n^3), where n is total number of elements 
 *
